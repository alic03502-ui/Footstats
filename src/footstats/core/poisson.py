...
def _macierz(
    ...
    rows_, cols_ = np.unravel_index(
        top5_raw,
        M.shape,
    )

    top5_data = tuple(
        (int(r), int(c), float(M[r, c]))
...
def _baza_ligowa(
    ...
    cache = df_mecze.attrs.setdefault(
        _ATR_SILY_LIG,
        {},
    )
    ...
    if liga not in cache:
        ...
        cache[liga] = sily_ligowe(
            df_mecze[
                df_mecze["league"] == liga
            ]
        )
...
def predict_match(
    ...
    importance_g = importance_g or {
        "bonus_atak": 1.0,
        "komentarz": "",
        "status": "NORMAL",
    }

    importance_a = importance_a or {
        "bonus_atak": 1.0,
        "komentarz": "",
        "status": "NORMAL",
    }

    heurystyka_g = heurystyka_g or {
        "mnoznik_atak": 1.0,
        "mnoznik_obr": 1.0,
        "opis": "",
    }

    heurystyka_a = heurystyka_a or {
        "mnoznik_atak": 1.0,
        "mnoznik_obr": 1.0,
        "opis": "",
    }
    ...
    fortress_g = fortress_g or {
        "bonus_obrona": 1.0,
        "fortress": False,
        "opis": "",
    }

    klasyfikacja = klasyfikacja or {
        "typ": "LIGA",
        "rewanz": False,
        "single": False,
        "agg_g": None,
    ...
    is_liga = (
        stage_up == "REGULAR_SEASON"
    )

    jest_rewanz = klasyfikacja.get(
        "rewanz",
        False,
    )

    jest_single = klasyfikacja.get(
        "single",
        False,
    )

    # ── Zbierz mecze do analizy ──────────────────────────────────────
    g_h, a_h = _kanoniczne_nazwy(
        df_mecze,
        g,
        a,
    )
    ...
    maska = (
        df_mecze["gospodarz"].isin(
            (g_h, a_h)
        )
        | df_mecze["goscie"].isin(
            (g_h, a_h)
        )
    )
    ...
    if len(df_f) < 4:
        ...
        sily_all, srednia_all = _oblicz_sile_wazona(
            df_f_all
        )
        ...
        sily, srednia = _oblicz_sile_wazona(
            df_f
        )
    ...
    jest_knockout = _czy_knockout(
        stage
    )

    if jest_rewanz:
        agg_g = klasyfikacja.get(
            "agg_g"
        )

        agg_a = klasyfikacja.get(
            "agg_a"
        )

        if (
            agg_g is not None
            and agg_a is not None
        ):
            (
                lambda_g,
                lambda_a,
            ...
            (
                ...
                first_leg_g,
                first_leg_a,
                jest_gospodarzem_1=False,
            )

    elif (
        jest_knockout
        and first_leg_g is not None
    ):
        (
            lambda_g,
            lambda_a,
    ...
    # λ_dom = atak gospodarza (xGF) ZDERZONY z obroną gościa (xGA).
    ...
    if use_xg:
        try:
            ...
            # use the season corresponding to the prediction date.
            if prediction_date is not None:
                _prediction_dt = pd.to_datetime(
                    prediction_date
                )
            ...
            # Gość strzela:
            # jego atak vs obrona gospodarza
            if (
                a_xgf
                and a_xgf > 0
            ):
            ...
            # xG cache niedostępny → czyste lambdy Poissona
            pass
    ...
    pewnosc = AnalizaH2H.oblicz_pewnosc_laczna(
        n_h2h_srednia,
        len(df_f),
    )
